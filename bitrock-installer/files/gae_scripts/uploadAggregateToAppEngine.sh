...
cd "$UPLOAD_ROOT"
	( ( ( "$UPLOAD_ROOT/appengine-java-sdk/bin/appcfg.sh" --email=${EMAIL} --passin backends LegacyRemoval list 2>&1 && OUTCOME=0 && echo ---LISTING OF BACKENDS-SCRIPT_SUCCESS--- ) || echo ---LISTING OF BACKENDS-SCRIPT-FAILURE--- ) | sed -e"/assword fo/s/.*//" ) << __THE__END__
${PASSWD}
__THE__END__
	( ( ( "$UPLOAD_ROOT/appengine-java-sdk/bin/appcfg.sh" --email=${EMAIL} --passin backends ODKAggregate delete background 2>&1 && OUTCOME=0 && echo ---REMOVAL OF BACKENDS-SCRIPT_SUCCESS--- ) || echo ---REMOVAL OF BACKENDS-SCRIPT-FAILURE--- ) | sed -e"/assword fo/s/.*//" ) << __THE__END__
${PASSWD}
__THE__END__
