...
public class SubmissionTabUI extends TabPanel {
  ...
  public SubmissionTabUI(AggregateUI baseUI) {
    ...
    for (int i = 0; i < SUBMISSION_MENU.length; i++)
      this.getTabBar().getTab(i)
          .addClickHandler(getSubMenuClickHandler(SUBMISSIONS, SUBMISSION_MENU[i]));
  }
  ...
  public void setupFormsAndGoalsPanel() {
    ...
    visualizeButton.addClickHandler(new ClickHandler() {
      @Override
      public void onClick(ClickEvent event) {
        final PopupPanel vizPopup = new CreateNewVisualizationPopup(getHeaders(), getSubmissions(),
            currentGroup.getFormId(), formSvc, submissionSvc, baseUI);
    ...
    exportButton.addClickHandler(new ClickHandler() {
      @Override
      public void onClick(ClickEvent event) {
        final PopupPanel popup = new CreateNewExportPopup(currentGroup.getFormId(), formSvc, baseUI
            .getManageNav(), baseUI);
    ...
    publishButton.addClickHandler(new ClickHandler() {
      @Override
      public void onClick(ClickEvent event) {
        final PopupPanel popup = new CreateNewExternalServicePopup(currentGroup.getFormId(), baseUI
            .getManageNav().servicesAdminSvc, hash, baseUI);
        popup.setPopupPositionAndShow(new PopupPanel.PositionCallback() {
          @Override
          public void setPosition(int offsetWidth, int offsetHeight) {
            int left = ((Window.getClientWidth() - offsetWidth) / 2);
  ...
  ClickHandler getSubMenuClickHandler(final String menu, final String subMenu) {
  ...
  public HorizontalPanel setupFiltersDataPanel(List<FilterGroup> groups) {
    ...
    newFilter.addClickHandler(new ClickHandler() {
      @Override
      public void onClick(ClickEvent event) {
        final CreateNewFilterPopup filterPopup = new CreateNewFilterPopup(dataTable, currentGroup,
            baseUI);
    ...
    filterPanel.getElement().getFirstChildElement().getFirstChildElement().getFirstChildElement()
        .setId("filters_panel");
  ...
  /**
   * NOTE: This formatting function is called by several places, should not be
  ...
  public void updateSubmissionTable(FlexTable table, SubmissionUISummary summary) {
    ...
    for (SubmissionUI row : tableSubmissions) {
      ...
      for (final String values : row.getValues()) {
        ...
        case BINARY:
          if (values == null) {
            table.setText(i, j, UIConsts.EMPTY_STRING);
          } else {
            Image image = new Image(values + UIConsts.PREVIEW_SET);
            image.addClickHandler(new ClickHandler() {
              @Override
              public void onClick(ClickEvent event) {
                final PopupPanel popup = new BinaryPopup(values);
                popup.setPopupPositionAndShow(new PopupPanel.PositionCallback() {
                  @Override
                  public void setPosition(int offsetWidth, int offsetHeight) {
                    int left = ((Window.getClientWidth() - offsetWidth) / 2);
        ...
        case REPEAT:
          if (values == null) {
            ...
            final AggregateUI tmp = baseUI; // fix after refactoring of the
                                            // function and global services
