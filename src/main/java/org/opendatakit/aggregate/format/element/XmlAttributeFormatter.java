...
public class XmlAttributeFormatter implements ElementFormatter {
  ...
  public void formatDate(Date date, FormElementModel element, String ordinalValue, Row row) {
    addToXmlValueToRow(Optional.ofNullable(date).map(JRTemporal::date).map(JRTemporal::getRaw).orElse(null), asAttributeName(element), row);
  }

  @Override
  public void formatDateTime(Date date, FormElementModel element, String ordinalValue, Row row) {
    addToXmlValueToRow(Optional.ofNullable(date).map(JRTemporal::dateTime).map(JRTemporal::getRaw).orElse(null), asAttributeName(element), row);
  }

  @Override
  public void formatTime(Date date, FormElementModel element, String ordinalValue, Row row) {
    addToXmlValueToRow(Optional.ofNullable(date).map(JRTemporal::time).map(JRTemporal::getRaw).orElse(null), asAttributeName(element), row);
  }
  ...
  public void formatJRDate(JRTemporal value, FormElementModel element, String ordinalValue, Row row) {
    addToXmlValueToRow(Optional.ofNullable(value).map(JRTemporal::getRaw).orElse(null), asAttributeName(element), row);
  }
