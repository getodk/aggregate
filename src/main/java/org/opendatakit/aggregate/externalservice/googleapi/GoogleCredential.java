 ...
 *
 * Revised to use OpenDataKit version of hand-coded OAuth2 token request as the implementation
 * in the 1.12.beta seems to be faulty against Google's own APIs???
 *
 *  @author mitchellsundt@gmail.comS
 */
public class GoogleCredential extends Credential {
  ...
  // @Override
  protected TokenResponse executeRefreshToken2() throws IOException {
    if (serviceAccountPrivateKey == null) {
  ...
  protected TokenResponse executeRefreshToken() throws IOException {
    ...
    jwtBody.put("iss", serviceAccountUser);
    jwtBody.put("scope", serviceAccountScopes);
...

    {
       ...
       try {
          signer.initSign(serviceAccountPrivateKey);
    ...
    try {
       ...
       if (statusCode != HttpStatus.SC_OK) {
          ...
          if (entity != null
             ...
             tr.setFactory(this.getJsonFactory());
             if ( "Bearer".equals(tokenType)) {
               tr.setTokenType(tokenType);
               tr.setAccessToken(accessToken);
               if ( userData.containsKey("refresh_token") ) {
                 tr.setRefreshToken((String)userData.get("refresh_token"));
               }
               if ( userData.containsKey("expires_in") ) {
                 tr.setExpiresInSeconds(Long.valueOf(userData.get("expires_in").toString()));
               }
               tr.setScope(serviceAccountScopes);
               return tr;
             }
             throw new IllegalArgumentException(
