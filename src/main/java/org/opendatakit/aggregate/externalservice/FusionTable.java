 ...
 * 
 */
public class FusionTable extends OAuth2ExternalService implements
		ExternalService {
	private static final Log logger = LogFactory.getLog(FusionTable.class
			.getName());
	...
	/**
	 * Datastore entity specific to this type of external service for the
	 * repeats
	 */
	 ...
	 * @param cc
	 */
	private FusionTable(FusionTable2ParameterTable entity,
			FormServiceCursor formServiceCursor, IForm form, CallingContext cc) {
		super(form, formServiceCursor, new FusionTableElementFormatter(
	...
	private FusionTable(FusionTable2ParameterTable entity, IForm form,
		...
		this(entity, createFormServiceCursor(form, entity,
				externalServiceOption, ExternalServiceType.GOOGLE_FUSIONTABLES,
				cc), form, cc);

		// and create records for all the repeat elements (but without any
		// actual
		// table ids)...
		Datastore ds = cc.getDatastore();
		User user = cc.getCurrentUser();
		FusionTable2RepeatParameterTable frpt = FusionTable2RepeatParameterTable
				.assertRelation(cc);

		for (FormElementModel repeatGroupElement : form
				.getRepeatGroupsInModel()) {
			FusionTable2RepeatParameterTable t = ds.createEntityUsingRelation(
					frpt, user);
			t.setUriFusionTable(objectEntity.getUri());
			t.setFormElementKey(repeatGroupElement
					.constructFormElementKey(form));
	 ...
	 * @throws ODKDatastoreException
	 */
	public FusionTable(FormServiceCursor formServiceCursor, IForm form,
			CallingContext cc) throws ODKDatastoreException {

		this(retrieveEntity(FusionTable2ParameterTable.assertRelation(cc),
				formServiceCursor, cc), formServiceCursor, form, cc);

		repeatElementEntities.addAll(FusionTable2RepeatParameterTable
				.getRepeatGroupAssociations(objectEntity.getUri(), cc));
	}
	 ...
	 * @throws ODKDatastoreException
	 */
	public FusionTable(IForm form,
			ExternalServicePublicationOption externalServiceOption,
			String ownerEmail, CallingContext cc)
	 ...
	 * @throws ODKDatastoreException
	 */
	private static final FusionTable2ParameterTable newFusionTableEntity(
			String ownerEmail, CallingContext cc) throws ODKDatastoreException {
		Datastore ds = cc.getDatastore();
		User user = cc.getCurrentUser();
	...
	protected String getOwnership() {
		return objectEntity.getOwnerEmail().substring(
				EmailParser.K_MAILTO.length());
	}
	...
	protected String getAccessToken(boolean forceRefresh, CallingContext cc)
		...
		try {
			if (accessToken == null && !forceRefresh) {
				accessToken = ServerPreferencesProperties
						.getServerPreferencesProperty(
								cc,
	...
	public void initiate(CallingContext cc) throws ODKExternalServiceException,
		...
		if (fsc.isExternalServicePrepared() == null
				|| !fsc.isExternalServicePrepared()) {

			if (objectEntity.getFusionTableId() == null) {
				String tableId = executeFusionTableCreation(
						form.getTopLevelGroupElement(), cc);
				objectEntity.setFusionTableId(tableId);
			}

			// See which of the repeat groups still need to have their tableId
			// created
			// and define those...
			for (FormElementModel repeatGroupElement : form
					.getRepeatGroupsInModel()) {
				...
				for (FusionTable2RepeatParameterTable t : repeatElementEntities) {
					if (objectEntity.getUri().equals(t.getUriFusionTable())
							&& repeatGroupElement.constructFormElementKey(form)
									.equals(t.getFormElementKey())) {
						// Found the match
						if (found) {
						...
						if (t.getFusionTableId() != null) {
							String id = executeFusionTableCreation(
									repeatGroupElement, cc);
				...
				if (!found) {
					throw new ODKExternalServiceException(
							"missing row in FusionTableRepeatParameterTable");
				}
			}
			persist(cc);
		...
		if (!fsc.getExternalServicePublicationOption().equals(
			...
			UploadSubmissions uploadTask = (UploadSubmissions) cc
	...
	public void sharePublishedFiles(String ownerEmail, CallingContext cc)
		...
		executeDrivePermission(objectEntity.getFusionTableId(),
				objectEntity.getOwnerEmail(), logger, "fusion table", cc);
	...
	protected void insertData(Submission submission, CallingContext cc)
		...
		for (FusionTable2RepeatParameterTable tableId : repeatElementEntities) {
			...
			FormElementModel element = FormElementModel
					.retrieveFormElementModel(form, elementKey);
				...
				if (value instanceof RepeatSubmissionType) {
					...
					if (repeat.getElement().equals(element)) {
						for (SubmissionSet set : repeat.getSubmissionSets()) {
							executeInsertData(tableId.getFusionTableId(), set,
									headers, cc);
						}
					}
				} else {
	...
	private void executeInsertData(String tableId, SubmissionSet set,
			List<String> headers, CallingContext cc)
			throws ODKExternalServiceException {
		...
		try {
			...
			String insertQuery = FusionTableConsts.INSERT_STMT + tableId
					+ createCsvString(headers.iterator())
					+ FusionTableConsts.VALUES_STMT
			...
			try {
				...
				e1.printStackTrace();
				throw new ODKExternalServiceException(
						"Unable to set OperationalStatus to Bad credentials: "
								+ e1);
			}
			throw e;
	...
	private String executeFusionTableCreation(FormElementModel root,
			CallingContext cc) throws ODKExternalServiceException {

		String resultRequest;
		try {
			String createStmt = createFusionTableStatement(form, root);
			resultRequest = executeStmt(POST, FUSION_TABLE_TABLE_API,
					createStmt, null, cc);
		} catch (ODKExternalServiceException e) {
		...
		try {
			...
			Map<String, Object> result = mapper.readValue(resultRequest,
					Map.class);
	...
	private String createFusionTableStatement(IForm form,
			FormElementModel rootNode) throws JsonGenerationException,
		...
		tableResource.put("description", form.getViewableName() + " "
				+ timestamp + " - " + rootNode.getElementName());
		tableResource.put("isExportable", true);

		List<String> headers = headerFormatter.generateHeaders(form, rootNode,
				null);
		...
		for (int i = 0; i < headers.size(); ++i) {
			...
			colResource.put("type", FusionTableConsts.typeMap.get(type)
					.getFusionTypeValue());
	...
	public String getDescriptiveTargetString() {
		...
		return HtmlUtil.createHrefWithProperties(
				"http://www.google.com/fusiontables/DataSource", properties,
				"View Fusion Table", true);
	}
