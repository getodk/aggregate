...
public class DbTableFiles extends AbstractBlobRelationSet {
   ...
   * ODKDatastoreException { return new DbTableFiles(cc); }
   */

  /*
   * /*** these will be the column names in the datastore table *** public
   * static final String TABLE_ID = "TABLE_ENTRY_ID"; // this will house the
   * different key possibilities, such as // listview, boxview, etc. public
   * static final String FILE_KEY = "FILE_KEY"; // will house the actual file.
   * public static final String FILE = "FILE";
   *
   *
   * // name of the table private static final String RELATION_NAME =
   * "TABLE_FILES";
   *
   * // the "columns" themselves. columns are fields within an // entity (row).
   * private static final List<DataField> dataFields;
   *
   * // do the one time constructor stuff, but do it in a static block // so it
   * is more efficient. a la Dylan in DbColumn.java. static { dataFields = new
   * ArrayList<DataField>(); dataFields.add(new DataField(TABLE_ID,
   * DataType.STRING, false) .setIndexable(IndexType.HASH)); dataFields.add(new
   * DataField(FILE_KEY, DataType.STRING, false)); dataFields.add(new
   * DataField(FILE, DataType.BINARY, false)); }
   *
   * public static Relation getRelation(CallingContext cc) throws
   * ODKDatastoreException { Relation relation = new Relation(RUtil.NAMESPACE,
   * RELATION_NAME, dataFields, cc); return relation; }
   *
   ...
   * ODKDatastoreException { return
   * getRelation(cc).query("DbTableFiles.query()", cc).equal(TABLE_ID, tableId)
   * .execute(); }
   */
