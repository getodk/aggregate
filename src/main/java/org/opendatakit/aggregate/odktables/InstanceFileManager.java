...
public class InstanceFileManager {
  ...
  /**
   * Retrieve the content info for a given file.
   * Access to the blob entity set needs to be guarded by a task lock.
   * 
   * @param tableId
   * @param rowId
   * @param partialPath
  ...
  public FileContentInfo getFile(String tableId, String rowId, String partialPath,
    ...
    try {
      ...
      LockTemplate propsLock = new LockTemplate(tableId, rowId, ODKTablesTaskLockType.TABLES_NON_PERMISSIONS_CHANGES, cc);
      try {
        propsLock.acquire();
        ...
        for (int i = 1; i <= count; ++i) {
          ...
          if (path != null && path.equals(partialPath)) {
            ...
            if (fileBlob != null && contentType != null && contentLength != null
              ...
              FileContentInfo fo = new FileContentInfo(path, contentType,
                  contentLength, contentHash,
                  fileBlob);
              return fo;
            } else {
              return null;
  ...
  /**
   * Save a given file content under this tableId and rowId.
   * Manipulations of the blob entity set (which is being updated) needs to be guarded by a task lock.
   * 
   * @param tableId
   * @param rowId
   * @param fi
  ...
  public InstanceFileChangeDetail putFile(String tableId, String rowId, FileContentInfo fi,
      TablesUserPermissions userPermissions) throws ODKDatastoreException, ODKTaskLockException, PermissionDeniedException {
    ...
    try {
      ...
      try {
        ...
        BlobSubmissionOutcome outcome = instance
            .addBlob(fi.fileBlob, fi.contentType, fi.partialPath, false, cc);
  ...
  /**
   * Retrieve the BlobEntitySet for a given tableId and rowId.
   * This should be treated as read-only by the caller.
   * Updates should be done through the putFile or postFiles APIs. 
   * Manipulations of the blob entity set (which is being updated) needs to be guarded by a task lock.
   * 
  ...
  public void getInstanceAttachments(String tableId, String rowId, FileContentHandler cb,
      TablesUserPermissions userPermissions) throws IOException, ODKTaskLockException, PermissionDeniedException, ODKDatastoreException {
      ...
      try {
        ...
        for (int i = 1; i <= count; ++i) {
          ...
          FileContentInfo info = new FileContentInfo( instance.getUnrootedFilename(i, cc),
              instance.getContentType(i, cc),
              instance.getContentLength(i, cc), instance.getContentHash(i, cc), null);
  ...
  public void postFiles(String tableId, String rowId, InMultiPart inMP,
      TablesUserPermissions userPermissions) throws IOException, ODKTaskLockException, ODKTablesException, ODKDatastoreException {
     ...
     try {
       ...
       try {
         ...
         while (inMP.hasNext()) {
           ...
           if (partialPath == null) {
             e = new ODKTablesException(InstanceFileService.ERROR_MSG_MULTIPART_CONTENT_FILENAME_EXPECTED);
             continue;
           }
           ...
           // we are adding one or more files -- delete any cached ETag value for this row's attachments manifest
           try {
             DbTableInstanceManifestETagEntity entity = DbTableInstanceManifestETags.getRowIdEntry(tableId, rowId, cc);
             entity.delete(cc);
           ...
           for (int i = 1; i <= count; ++i) {
             ...
             if (path != null && path.equals(partialPath)) {
               ...
               if (md5Hash.equals(instance.getContentHash(i, cc))) {
                 ...
                 found = true;
                 e = new InstanceFileModificationException(ERROR_FILE_VERSION_DIFFERS + "\n" + partialPath);
                 break;
               }
             }
           }
