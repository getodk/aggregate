...
/**
 * SynchronizedAPI contains API calls for using Aggregate as a synchronization service for tables.
 */
public class SynchronizedAPI extends CommonAPI
{

    /**
     * Constructs a new instance of SynchronizedAPI, using the supplied user identification for API calls which require it.
     *
     ...
     *
     * @param tableID the client's unique identifier for the table
     ...
     * @param tableID the unique identifier that the caller will use to identify the table
     * @return the current Modification of the table. The list returned by getRows() will be populated with rowUUID, revisionNumber, and data for the row. Make sure that all of this data is stored as it will be required for other API calls (see {@link SynchronizedAPI the top of this file} for a summary of client requirements for synchronized API usage). 
     * @throws TableAlreadyExistsException if the caller has already registered a table with tableID
     ...
     *
     * @param tableID the caller's identifier for the table
     * @param modificationNumber the current modificationNumber of the caller's copy of the table
     * @param newRows a list of rows to insert. These should be populated with rowIDs and data.
     * @return a Modification whose modificationNumber represents the latest modification of the table in Aggregate. Calling getRows() on the Modification will return a list of rows where each row is populated with rowID, rowUUID, and revisionNumber. Make sure that all of this data is stored as it will be required for other API calls (see {@link SynchronizedAPI the top of this file} for a summary of client requirements for synchronized API usage). 
     * @throws OutOfSynchException if the given modificationNumber does not match the modificationNumber of the table in Aggregate. In this case the caller should call {@link #synchronize}, then attempt to insert again.
     ...
     * @param tableID the caller's identifier for the table
     * @param modificationNumber the current modificationNumber of the caller's copy of the table
     * @param changedRows a list of synchronized rows which are populated rowUUIDs, revisionNumbers,and data. These rows must already exist in Aggregate's copy of the table
     * @return a Modificaton whose modificationNumber represents the latest modification of the table in Aggregate. Calling getRows() on the Modification will return a list of rows where each row is populated with rowUUID and revisionNumber. Make sure that all of this data is stored as it will be required for other API calls (see {@link SynchronizedAPI the top of this file} for a summary of client requirements for synchronized API usage). 
     * @throws OutOfSynchException if the given modificationNumber does not match the modificationNumber of the table in Aggregate. In this case the caller should call {@link #synchronize}, then attempt to insert again.
     * @throws TableDoesNotExistException if the caller has no table with tableID registered for synchronization
     * @throws PermissionDeniedException if the userID used to make the API call does not have write permission on the table
     */
     ...
     *
     * @param tableID the caller's identifier for the table
     * @param modificationNumber the current modificationNumber of the caller's copy of the table. The caller must be up to date with this modificationNumber in Aggregate.
     * @return a Modification whose modificationNumber represents the latest modification of the table in Aggregate. Calling getRows() on the Modification will return a list of rows where each row is populated with rowUUID, revisionNumber, and data. Make sure that all of this data is stored as it will be required for other API calls (see {@link SynchronizedAPI the top of this file} for a summary of client requirements for synchronized API usage). 
     * @throws TableDoesNotExistException if the caller has no table with tableID registered for synchronization
     * @throws PermissionDeniedException if the userID used to make the API call does not have read permission on the table
     */
