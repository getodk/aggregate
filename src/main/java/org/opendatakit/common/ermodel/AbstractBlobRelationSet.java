...
/**
 * Base class for manipulating blob sets.  The constructors assume that the
 * base name of the table is UPPER_CASE only.
 * 
...
public class AbstractBlobRelationSet implements BlobRelationSet {
	 ...
	 * table names in the datastore will have 3 leading underscores.  3 tables will be created
	 * with differing suffixes.  
	 * @param cc
	 * @throws ODKDatastoreException
	 */
	protected AbstractBlobRelationSet( String tableName, CallingContext cc) throws ODKDatastoreException {
	 ...
	 * the namespace string, 2 underscores, and this tableName string.  3 tables will be created
	 * with differing suffixes.  
	 * @param cc
	 * @throws ODKDatastoreException
	 */
	protected AbstractBlobRelationSet(String namespace, String tableName, CallingContext cc) throws ODKDatastoreException {
		...
		if ( backingBaseTableName.length() > Relation.MAX_PERSISTENCE_NAME_LENGTH - 4) {
			throw new IllegalArgumentException("Backing base table name is too long: " + backingBaseTableName);
		}
		this.namespace = TableNamespace.EXTENSION;
		initialize(cc);
	}
	 ...
	 * @throws ODKDatastoreException 
	 */
	protected AbstractBlobRelationSet( TableNamespace type, String tableName, CallingContext cc) throws ODKDatastoreException {
	...
	/**
	 * Create a new blob entity.  This entity does not exist in the database
	...
	public BlobEntitySet newBlobEntitySet(CallingContext cc) throws ODKDatastoreException {
		...
		return new BlobEntitySetImpl( uri, uri, new BinaryContentManipulator( uri, uri,
	 ...
	 * until you put() it there.
	 * 
	 * @param uri primary key of this Blob set.
	 * @param cc
	 * @return
	 * @throws ODKDatastoreException 
	 */
	...
	/**
	 * Get an existing blob entity.  This API is useful for retrieving 
	 * Blob sets in the SUBMISSIONS or INTERNALS table namespaces. 
	 * 
	 * @param uri primary key of this Blob set.
	 * @param topLevelUri primary key of the top-level record linked to this Blob set.
	 * @param cc
	 * @return
	 * @throws ODKDatastoreException 
	 */
	@Override
	public BlobEntitySet getBlobEntitySet(String uri, String topLevelUri, CallingContext cc) throws ODKDatastoreException {
		// Note that we refer to the BinaryContentManipulator's parentAuri as 
		// the uri of the blob set.
	...
	@Override
	public void deleteBlobEntitySets( List<BlobEntitySet> eList, CallingContext cc ) throws ODKDatastoreException {
		for ( BlobEntitySet e : eList ) {
			e.remove(cc);
		}
	}
	...
	public static class BlobEntitySetImpl implements BlobEntitySet {
		...
		public String getContentHash(int ordinal) {
			return m.getContentHash(ordinal);
		}

		@Override
		public Long getContentLength(int ordinal) {
			return m.getContentLength(ordinal);
		}

		@Override
		public String getContentType(int ordinal) {
			return m.getContentType(ordinal);
		}

		@Override
		public String getUnrootedFilename(int ordinal) {
			return m.getUnrootedFilename(ordinal);
		}
		...
		public BlobSubmissionOutcome addBlob(byte[] byteArray,
				...
				throws ODKDatastoreException {
			return m.setValueFromByteArray(byteArray, contentType,
					contentLength, unrootedFilePath, cc);
		}
		...
		protected BlobEntitySetImpl(String uri, String topLevelUri, BinaryContentManipulator m, CallingContext cc) throws ODKDatastoreException {
			this.uri = uri;
			this.topLevelUri = topLevelUri;
			this.m = m;
			this.m.refreshFromDatabase(cc);
		}
		...
		public BlobSubmissionOutcome addBlob(byte[] byteArray,
				...
				throws ODKDatastoreException {
			return addBlob( byteArray, contentType, Long.valueOf( byteArray.length), 
							unrootedFilePath, cc);
		}
	}
