...
public class Relation {
   ...
   * 
   * @param namespace
   *          the namespace the relation should live under. See above for the
   *          constraints on namespaces.
   * @param name
   *          the name of the relation. See above for the constraints on names.
   * @param attributes
   *          the attributes the relation will have. Must not be null or empty.
   * @param cc
   *          the context. Must not be null.
   * @throws ODKDatastoreException
   *           if there is a problem communicating with the datastore
   */
  public Relation(String namespace, String name, List<Attribute> attributes, CallingContext cc)
      throws ODKDatastoreException {
   ...
   * @throws ODKDatastoreException
   */
  protected Relation(String namespace, String name, CallingContext cc) throws ODKDatastoreException {
    Check.notNullOrEmpty(namespace, "namespace");
    Check.notNullOrEmpty(name, "name");
    Check.notNull(cc, "cc");
    ...
    if (attributes.isEmpty()) {
      throw new ODKDatastoreException(String.format(
          "Relation '%s' in namespace '%s' does not exist.", name, namespace));
    }
    List<DataField> fields = new ArrayList<DataField>();
    for (Attribute attribute : attributes) {
      fields.add(attribute.toDataField());
  ...
  /**
   * @param attributeName
   *          the valid name of an Attribute on this Relation. Must not be null
   *          or empty.
   * @return
   */
   ...
   */
  private List<Attribute> getAttributes(String namespace, String name, CallingContext cc)
      throws ODKDatastoreException {
    ...
    if (name.equals(AttributeRelation.name())) {
      ...
      List<Entity> attrEntities = getAttributeEntities(namespace, name, cc);

      // convert Entities to Attributes
      attributes = new ArrayList<Attribute>();
      for (Entity attrEntity : attrEntities) {
        String attributeName = attrEntity.getString(AttributeRelation.NAME);
        AttributeType attributeType = AttributeType.valueOf(attrEntity
            .getString(AttributeRelation.TYPE));
        boolean attributeNullable = attrEntity.getBoolean(AttributeRelation.NULLABLE);
        Attribute attribute = new Attribute(attributeName, attributeType, attributeNullable);
  ...
  /**
   * Retrieves a list of all entities in the AttributeRelation which are defined
   * for the given namespace and name.
   */
  private List<Entity> getAttributeEntities(String namespace, String name, CallingContext cc)
      throws ODKDatastoreException {
    AttributeRelation attributeRelation = AttributeRelation.getInstance(namespace, cc);
   ...
   *           if there is no Entity with the given identifier stored in this
   *           Relation.
   */
  public Entity getEntity(String aggregateIdentifier) throws ODKDatastoreException {
    Check.notNullOrEmpty(aggregateIdentifier, "aggregateIdentifier");
    return Entity.fromEntity(relation, relation.getEntity(aggregateIdentifier, relation.getCC()));
  }
  ...
  public void dropRelation() throws ODKDatastoreException {
    for (Entity attribute : getAttributeEntities(namespace, name, relation.getCC())) {
      attribute.delete();
    }
    try {
      relation.dropRelation(relation.getCC());
    } catch (NullPointerException e) {
      throw new ODKDatastoreException("Relation does not exist in datastore.", e);
    }
  }
   ...
   * {@link #Relation(String, String, List, CallingContext) Relation} regarding
   * constraints on namespaces and names.
   * 
   * @param namespace
