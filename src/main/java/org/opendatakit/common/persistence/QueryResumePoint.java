 ...
 * Resumable queries must specify at least one sort
 * column.  The first sort specified is the dominant sort.
 * 
 * <p>If not already defined, the persistence layer
 * will add a sort against the PK in the same direction
 * (ascending/descending) as the dominant sort.
 * 
 * <p>The resume point tracks the dominant sort column
 * name, the last value of this column from the prior 
 * query, and the PK of the last value returned by the
 * prior query.
 * 
 * <p>The resume point can be used against the same Query
 * or you can construct a new query with inverted sort 
 * criteria (keeping the same sequence of sorted columns
 * but reversing the sort directions of each) and obtain
 * the values preceding the resume point.
 * 
