...
public final class FormElementModel {
	 ...
	 * This should have a parallel structure to the SubmissionKey, which uses the 
	 * getGroupQualifiedElementName() (above) and the collapsed entity sets of the 
	 * SubmissionSet for its construction (i.e., they are different construction 
	 * methodologies, so there is a risk of divergence).
	 * 
	 * @param form
	...
	private final String getFullyQualifiedElementName(Form form) {
		...
		if (getParent() == null) {
			...
			if ( getParent().getElementType() == ElementType.REPEAT ||
	...
	public static final FormElementModel retrieveFormElementModel(Form form, FormElementKey key)  {
		...
		while ( slashParts.length > slashPosition ) {
			...
			int colonPosition = 0;
			if ( first ) {
				// first time through, the top level group element is the first colon part
				// all other times, we need to search to find a match for this first part.
