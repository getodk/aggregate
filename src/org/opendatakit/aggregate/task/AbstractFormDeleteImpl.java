...
public abstract class AbstractFormDeleteImpl implements FormDelete {
  ...
  public final void deleteForm(Form form, User user) throws ODKDatastoreException,
      ODKFormNotFoundException, ODKExternalServiceDependencyException {
    Datastore ds = (Datastore) ContextFactory.get().getBean(BeanDefs.DATASTORE_BEAN);

    List<ExternalService> services = FormServiceCursor.getExternalServicesForForm(form, null, ds, user);
    if (services.size() != 0) {
    ...
    if (relation != null) {
      ...
      if (submissionEntities.size() > 0) {
        ...
        for (CommonFieldsBase en : submissionEntities) {
          keys.add(new SubmissionKey(form.getFormId(), form.getTopLevelGroupElement()
              .getElementName(), en.getUri()));
        }
